...
my_col_pal <- scales::col_factor(levels = unique(df_monthly$dispatch_mos), "viridis")
df_monthly$color <- my_col_pal(df_monthly$dispatch_mos)

plot3d(x = df_monthly$dispatch_mos, y = df_monthly$count, z = df_monthly$dispatch_year,
  col = df_monthly$color,
  type = 'l', lwd=4,
  ylim = c(0, 200),
  xlab="Month", ylab="n Cases", zlab="Year"
  )
