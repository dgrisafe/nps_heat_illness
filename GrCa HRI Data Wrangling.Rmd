...
I've completed up until the variables `last_name`, `first_name`.
...
df_csv <- data_csv %>% 
  ...
  mutate(
    ...
    # create composite address variables for both USA and nation without states
    address = ifelse(
...
df_csv[df_csv$prid=="66267999","age_disp"] <- 58 # taken from var age_in_years_calc
```
...
```{r}
df_csv %>% dim()
df_csv %>% select(-patient_activity) %>% duplicated() %>% sum()
df_csv %>% select(-patient_activity) %>% distinct %>% dim()
```
...
Simple tables for variables in the anaytic dataset